export function normalizeSchematic(apiData: any) {
  ...
  // If it has wires property (like wire details), convert to expected format
    ...
    return {
      ...
      masterComponents: wireData.masterComponents || [],
      components: (wireData.components || []).map((c: any) => ({
        id: c.id || c.code,
        label: c.label || c.name || c.id || c.code || 'Unknown',
        category: c.category || c.componentCategory || 'Component',
        shape: c.shape || 'rectangle',
        componentType: c.componentType,
        connectors: (c.connectors || []).map((conn: any) => ({
          id: conn.id,
      ...
      connections: (wireData.connections || []).map((con: any) => ({
        ...
        from: {
          componentId: con.from?.componentId,
          connectorId: con.from?.connectorId,
          cavity: con.from?.cavity,
          gender: con.from?.gender
        },
        to: {
          componentId: con.to?.componentId,
          connectorId: con.to?.connectorId,
          cavity: con.to?.cavity,
  ...
  return {
    code: apiData.code,
    name: apiData.name,
    masterComponents: apiData.masterComponents || [],
